...
22. project location True North adapter — `Document.ActiveProjectLocation.GetProjectPosition(XYZ.Zero).Angle` is read-only and converted from radians to a JSON-safe clockwise Project-North-to-True-North rotation shared by Forward / Reverse (implemented; Revit 2024.3 real-machine validation pending; latitude/longitude extraction remains out of scope)
