...
- **Project location and true north:** Revit runtime uses `Document.ActiveProjectLocation.GetProjectPosition(XYZ.Zero).Angle` as the read-only True North source of truth. The adapter converts the API radians into JSON-safe radians/degrees and applies the documented clockwise Project-North-to-True-North rotation without sign guessing. Forward and Reverse share this orientation contract. Pure-Python callers may provide an explicit resolved rotation. Never silently fall back to 0° when the Revit value is unavailable. `SiteLocation` latitude/longitude extraction remains out of scope; Player latitude/longitude remain authoritative.
