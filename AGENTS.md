...
- **Project location and true north:** Revit runtime uses `Document.ActiveProjectLocation.GetProjectPosition(XYZ.Zero).Angle` as the read-only True North source of truth. Revit 2024.3 real-machine validation established that a clockwise 30° Rotate True North setting yields raw/internal -30°, so the adapter must not negate the raw angle. Dynamo_Shadow's internal contract remains clockwise from Project North/model +Y to True North. Forward and Reverse share the normalized orientation contract. Pure-Python callers may provide an explicit resolved rotation. Never silently fall back to 0° when the Revit value is unavailable. `SiteLocation` latitude/longitude extraction remains out of scope; Player latitude/longitude remain authoritative.
...
- Equal-time shadow calculation requires explicit or preset-derived `average_ground_level_elevation_m`, `measurement_height_m`, `latitude`, and `longitude`; Revit runtime additionally requires a resolved Active Project Location True North rotation, while pure-Python tests may explicitly supply `true_north_deg`.
