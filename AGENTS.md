...
- **Project location and true north:** Revit runtime uses `Document.ActiveProjectLocation.GetProjectPosition(XYZ.Zero).Angle` as the read-only True North source of truth. Keep the raw API angle (directed from True North to Project North) separate from Dynamo_Shadow's clockwise Project-North/model-+Y-to-True-North contract and convert it explicitly by sign inversion. Forward and Reverse share the normalized orientation contract. Pure-Python callers may provide an explicit resolved rotation. Never silently fall back to 0° when the Revit value is unavailable. `SiteLocation` latitude/longitude extraction remains out of scope; Player latitude/longitude remain authoritative.
