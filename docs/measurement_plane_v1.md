...
## Footprint diagnostics relation

The measurement plane is now also reported as context for footprint extraction readiness. Footprint loop candidates remain Revit `raw_internal_units`, while the measurement plane diagnostic object is expressed in meters. Formal Revit unit conversion is not implemented in this PR, so any raw footprint / measurement-plane relationship is diagnostic only.

Shadow projection from footprint loops to the measurement plane is not implemented.
